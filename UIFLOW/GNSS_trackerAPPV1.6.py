...
print('GPS_test based on DX-GP10 GNSS module(AT6558) V1.6')
print('Zell beta version  FW @2024.10.13')
...
#(2024, 10, 13, 6, 11, 57, 33, 0)
#RTC time: 2024-10-13 11:57:33 Sun
#2024-10-13 11:57:33 Sun

#image_BG = M5Img(0, 0, "res/BMW_coupit BG3.png")
rgb.setColorFrom(6, 10, 0x33ff33)#Left
rgb.setColorFrom(1, 5, 0xff6600)
rgb.setBrightness(10)
title0 = M5Label('GNSS tracker APP v1.6', x=20,y=0,color=0x0FFFFF, font=FONT_UNICODE_24, parent=None)
...
def callback_timer3(_arg):
  ...
  #timerSch.timer.deinit()
  #timerSch.timer.dead = True
  ...
  if(0==Tmr_alive):
      ...
      try:
          ...
          if(0==(Tmr_OVF_cnt%GUI_time_refresh_Tmr_threshold)):
          ...
          if(Tmr_OVF_cnt>Tmr_threshold):
            ...
            Altitude_min = min(Altitude_min,Alt_var)
          ...
          if(error_cnt>error_rest_threshold):
              #reset sys
              print('#>:Too many error occured, rest now!')
              machine.reset()
      ...
      #print('#>:wait for Reinit Timer3!')
      Tmr_alive = 0
      #timerSch.timer.init(period=500, mode=timerSch.timer.ONE_SHOT, callback=callback_timer3)#not workling!
      #timerSch.timer.dead = False
  pass
...
#timerSch.timer.init(period=500, mode=timerSch.timer.ONE_SHOT, callback=callback_timer3)
Tmr_alive =0
while True:
  wdt.feed()
  #if( Tmr_alive ==0):
      #timerSch.timer.init(period=500, mode=timerSch.timer.ONE_SHOT, callback=callback_timer3)
      #Tmr_alive =1
      #print('#>:Reinit Timer3!'+str(Tmr_OVF_cnt))
  wait_ms(2)
