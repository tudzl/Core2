...
#V1.7D added BtnC press cnt to trigger MBUS power reset
#V1.7C improved distance calc function
...
print('GNSS_test based on DX-GP10 GNSS module(AT6558) V1.7D')
print('Zell beta version  FW @2024.11.01')
...
title0 = M5Label('GNSS tracker APP v1.7D', x=20,y=0,color=0x0FFFFF, font=FONT_UNICODE_24, parent=None)
...
def buttonA_wasPressed():
  # global params
  global uart1,Altitude_max,Altitude_min,Distance_sofar,Time_var,label_Altitude_MAX_Value,label_TimeValue
  print('>>BTN_A pressed!')
...
def buttonC_wasPressed():
  # global params
  global Speed_max,Speed_ave,error_cnt,uart1,label_SpeedMax,distance,label_DistanceValue,Btn_C_pressCNT
  Speed_max = 0
  distance = 0
  Btn_C_pressCNT = Btn_C_pressCNT+1
  label_SpeedMax.setText('Max:'+str(Speed_max))
  label_DistanceValue.setText(str("{:.1f}".format(distance) ))
  if (Btn_C_pressCNT%5==0):
    power.setBusPowerMode(1)
    wait_ms(100)
    #power.setBusPowerMode(0)
    print('>>BTN_C pressed multiple times, MBUS power OFF!')
  if (Btn_C_pressCNT%6==0):
    #power.setBusPowerMode(1)
    power.setBusPowerMode(0)
    wait_ms(100)
    gps_0 = unit.get(unit.GPS, (14,13))
    print('>>BTN_C pressed multiple times, MBUS power ON!')
  print('>>BTN_C pressed, distance and Max speed cleared')
  pass
  ...
  if(0==Tmr_alive):
      try:
          ...
          if(0==Tmr_OVF_cnt%Tmr_threshold):
            ...
            #label_DistanceValue.setText(str(distance))
            label_DistanceValue.setText(str("{:.1f}".format(distance) ))
