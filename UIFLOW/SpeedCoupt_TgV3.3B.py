#Speedcoupit V3.3.PY  0162C86C core2 with 16MB Flash 8MB PSRAM
#V3.3 added filter for T_cycle, works great!?
#V3.2C: try read Sensor_pin in loops to assist trigger
#added pin interrupt, triggered to calc T_cycle2-->speed
#hall speed detect
...
gc.collect()
#check free space: The first two values of the tuple ar the block size, the 3rd the total number of blocks, the 4th the number of free blocks.
os.statvfs("")
T_interval = 450 #GUI renew
Wheel_length = 1510 #in cm, 20*1.50 　　1510 mm 　　EA062、FA073
Sensor_pin = 36 #PortB last pin 36
PWM_pin = 26
PWM_Freq = 4
screen = M5Screen()
screen.clean_screen()
...
pin1 = machine.Pin(Sensor_pin, mode=machine.Pin.IN, pull=None) #machine.Pin.PULL_DOWN  None Pin.PIN_HOLD keep status during deep sleep
#pin1 = machine.Pin(Sensor_pin, mode=machine.Pin.IN, pull=machine.Pin.PIN_HOLD)
PWM0 = machine.PWM(26, freq=PWM_Freq, duty=98, timer=0)
PWM_EN = 1
PWM0.pause()
image_BG = M5Img("res/BMW_coupit BG3.png", x=0, y=0, parent=None)
Title = M5Label('ID8 Speed Coupit demo V3.3', x=20, y=5, color=0x2517de, font=FONT_MONT_14, parent=None)
...
def touch_button_HighPrecision_pressed():
  ...
  if(PWM_EN):
    ...
    #touch_button_PWM = M5Btn(text='PWM', x=225, y=200, w=70, h=30, bg_c=0xFFFFFF, text_c#Speedcoupit V3.2.PY  0162C86C
#added pin interrupt, triggered to calc T_cycle2-->speed
#hall speed detect
#port B G26,G36: pin26 not working as digitalRead,
from m5stack import *
...
PWM0.pause()
image_BG = M5Img("res/BMW_coupit BG3.png", x=0, y=0, parent=None)
Title = M5Label('ID8 Speed Coupit demo V3.2', x=20, y=5, color=0x2517de, font=FONT_MONT_14, parent=None)
#Title.set_hidden(True)
#label_vol = M5Label('Vbus_In:', x=20, y=40, color=0xEE9E00, font=FONT_MONT_18, parent=None)
#label_Cur = M5Label('Current:', x=20, y=60, color=0xEE1E00, font=FONT_MONT_18, parent=None)
#label_Pow = M5Label('Power  :', x=20, y=80, color=0xCCCCCC, font=FONT_MONT_18, parent=None)

label_Hall = M5Label('Dist.:', x=170, y=40, color=0xEEAE00, font=FONT_MONT_14, parent=None)
label_Shift = M5Label('S3', x=98, y=116, color=0x2255EF, font=FONT_MONT_28, parent=None)
#label_Hall2 = M5Label('Hall2:', x=220, y=100, color=0xEE1E00, font=FONT_MONT_16, parent=None)
label_speed = M5Label('speed:', x=200, y=80, color=0x4EEE10, font=FONT_MONT_18, parent=None)
label_speed_num = M5Label('22', x=150, y=100, color=0xCEEEB0, font=FONT_MONT_40, parent=None)
...
T_interrupt_interval = time.ticks_ms()
#IRQ not working properly?
Trig_cnt = 0
T_cycle2 = 0
run_dist = 0
...
def event_interrupt(pin): #bugs, IRQ_FALLING, Sensor_pin still high
    global Hall_status,Trig_cnt,Sensor_pin,T_cycle2,T_interrupt_interval
    Hall_status=digitalRead(Sensor_pin)
    if(Hall_status==0):
      T_cycle2 = time.ticks_ms()-T_interrupt_interval
      Trig_cnt +=1
      T_interrupt_interval =  time.ticks_ms()
    #print(str(pin)+"Hall:"+str(Hall_status)+" INT trig.@"+str(Trig_cnt))
    #interrupt_source = pin
    #pass
pin1.irq(trigger=machine.Pin.IRQ_FALLING,handler=event_interrupt)#3 for both, IRQ_RISING,IRQ_FALLING
...
def event_interrupt(pin): #bugs, IRQ_FALLING, Sensor_pin still high
    ...
    #pass
pin1.irq(trigger=machine.Pin.IRQ_FALLING,handler=event_interrupt)#3 for both, IRQ_RISING,IRQ_FALLING
#https://gndtovcc.home.blog/2020/04/17/micropython-interrupts-with-esp32-and-esp8266/
#pin1.irq(trigger=machine.Pin.IRQ_RISING,handler=event_interrupt)
'''
'''
>>>touch = Pin(32, Pin.IN)
...
def buttonC_wasPressed():
  ...
  if (PWM_Freq<7):
      ...
      PWM0 = machine.PWM(26, freq=PWM_Freq, duty=96, timer=0)
  else:
      PWM0 = machine.PWM(26, freq=PWM_Freq, duty=98, timer=0)
  #INA_OK = 1 -INA_OK
...
print('Core2 bycycle Hall speed coupit V3.2B ')
print('Author: Zell, 08.June.2021')
...
if(INA_OK):
           ...
           ---readback config value: 0b110001011101111T_
           """
RTC_EN = 0
try:
...
while 1:
    ...
    '''
        ...
        if (Wheel_pos ==1):#&(GUI_renew==0)
          
          T_cycle = T_cycle_interval
          #if(debug_print_EN):
              #print (str(T_cycle))
          if (T_cycle>0):
            WheelSPD_tmp = Wheel_length/T_cycle #m/s
          #T_Hall = time.ticks_ms() # get millisecond counter
          #wait_ms(1)
          Wheel_pos = 0 
        elif(Wheel_run_cnt>2):#slow down
        ...
        if(GUI_renew==1):
          ...
          WheelSPD =WheelSPD_tmp;
    ''' #above is old algorithm for read IO status
        #new API for calc speed
    #Hall_status = digitalRead(Sensor_pin)
    # (Hall_status==0):#active input! 
    #if (Trig_cnt_pre<=Trig_cnt):#active input! tested not working well!

        #Trig_cnt_pre = Trig_cnt
    ...
    if(T_delta>=T_interval):
       ...
       if (T_cycle2>65):
          try:  
            T_cycle2= filter_T_cycle(T_cycle_pre,T_cycle2)
            WheelSPD = Wheel_length/T_cycle2 #m/s
            T_cycle_pre=T_cycle2
            Wheel_run_cnt = 0# test only
          except:
            print("WheelSPD calc error!"+str(T_cycle2))     
       ...
       Wheel_run_cnt = Wheel_run_cnt+1
       if (Wheel_run_cnt >4):
          #WheelSPD = Wheel_length/(T_cycle+T_delta*Wheel_run_cnt) #m/s #old api
          WheelSPD = Wheel_length/(T_cycle2+T_delta*Wheel_run_cnt) #m/s
          rgb.setColorAll(0x000000)
       if (Wheel_run_cnt >7):  
          WheelSPD = 0
       ...
       label_T_cycle.set_text ('CycleT:'+str(T_cycle2)+'ms')#test only
       label_Hall.set_text ('Dist.:'+str(run_dist)+"M")
       ...
       if(debug_print_EN):
          ...
          print('Trig_cnt:'+str(Trig_cnt))
          print('T_cycle2:'+str(T_cycle2))
          print('Sys cnt:'+str(sys_cnt))
          print('Sys fps:'+str(1000*sys_cnt/T_delta))
          print('sensor loop:'+str(T_loop_delta)+'us')
...
#power.setBusPowerMode(1) # turn on off MBUS 5V power
    ...
    #else: #no GUI renew
      #GUI_renew += 1
    T_loop_delta = time.ticks_diff(time.ticks_us(), T_loop)   
