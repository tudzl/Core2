...
def init_puzzle(Pixel_pos_init):
      #page 1
...
def pixel_row_col_update(Pixel_pos):
    ...
    Pixel_row_num =math.floor(Pixel_pos/8)+1
    if Pixel_pos>8:
        Pixel_col_num = math.fmod(Pixel_pos, 8)
    else:
        Pixel_col_num = Pixel_pos
    if Pixel_col_num ==0:
       Pixel_col_num =8
    pass   
...
def pixel_update(pos, pos_pre):
      ...
      neopixel0.setColor(pos, int(Color_Yellow/4))
      wait(0.05)
      neopixel0.setBrightness(Pixel_brightness_m)
      neopixel0.setColor(pos_pre, int(Color_Yellow/4))
...
neopixel0.setBrightness(Pixel_brightness)
label0 = M5TextBox(6, 10, "Pixel puzzle demo v0.91", lcd.FONT_DejaVu24,0xFFFFFF, rotate=0)
  ...
  #try:
      ...
      if Tilt_angel>12:
        ...
        if Pixel_pos == Pixel_target_index:
           ...
           try:
              #contain bugs!!!
              Pixel_target_index = math.fmod(run_cnt/63)+1
              Pixel_pos = random.randint(1, 63)+1
              label_pixel.setText("Pixel: "+str(Pixel_pos)+" Col:"+str(Pixel_col_num)+ "Target:"+str(Pixel_target_index))
           except:
